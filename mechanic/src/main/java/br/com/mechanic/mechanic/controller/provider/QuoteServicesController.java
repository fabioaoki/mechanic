...
import br.com.mechanic.mechanic.service.QuoteServiceBO;
...
public class QuoteServicesController {

    @Autowired
    private QuoteServiceBO quoteServiceBO;

//    @GetMapping
//    public ResponseEntity<Page<?>> findAll(
//            @RequestParam(defaultValue = "0") int page,
//            @RequestParam(defaultValue = "20") int size) {
//        return ResponseEntity.ok(quoteServiceBO.findAll(PageRequest.of(page, size)));
//    }

    @PostMapping("/provider/{providerAccountId}")
    @ResponseStatus(HttpStatus.CREATED)
    public ResponseEntity<?> createQuoteService(@PathVariable Long providerAccountId,
            @RequestBody QuoteServiceRequest quoteServiceRequest) throws QuoteServiceException, ProviderAccountException {
        try {
            log.info("Creating a quote service");
            return ResponseEntity.ok(quoteServiceBO.save(quoteServiceRequest, providerAccountId));
        } catch (QuoteServiceException e) {
            log.error("QuoteServiceException: {}", e.getMessage());
...
//    }
    ...
    private static ErrorResponse getErrorResponse(QuoteServiceException e) {
        ErrorResponse errorResponse = new ErrorResponse();
        errorResponse.setErrorCode(e.getErrorCode());
        errorResponse.setMessage(e.getMessage());
        return errorResponse;
